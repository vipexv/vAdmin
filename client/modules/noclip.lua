-- Since im lazy, NoClip Logic is a copy and pasta from: https://github.com/qbcore-framework/qb-adminmenu/blob/main/client/noclip.lua
...
local SetupCam = function()
    ...
    Camera = CreateCameraWithParams('DEFAULT_SCRIPTED_CAMERA', GetEntityCoords(NoClipEntity),
        vector3(0.0, 0.0, entityRot.z), 75.0)
    SetCamActive(Camera, true)
    RenderScriptCams(true, true, 1000, false, false)

    if PlayerIsInVehicle == 1 then
        AttachCamToEntity(Camera, NoClipEntity, 0.0, VehFirstPersonNoClip == true and 0.5 or -4.5,
            VehFirstPersonNoClip == true and 1.0 or 2.0, true)
    else
        AttachCamToEntity(Camera, NoClipEntity, 0.0, PedFirstPersonNoClip == true and 0.0 or -2.0,
            PedFirstPersonNoClip == true and 1.0 or 0.5, true)
    end
end
    ...
    CreateThread(function()
        while IsNoClipping do
            ...
            if IsControlAlwaysPressed(0, MOVE_FORWARDS) then
                ...
                if pitch.x >= 0 then
                    SetEntityCoordsNoOffset(NoClipEntity,
                        GetOffsetFromEntityInWorldCoords(NoClipEntity, 0.0, 0.5 * (Speed * multi),
